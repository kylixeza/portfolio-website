...
(function () {
    ...
    const Confetti = {
        ...
        fire() {
            ...
            // Sketchy bright colors
            const colors = ['#FFD93D','#FF6B6B','#00BFA6','#845EF7','#339AF0','#FF9EB5'];
        ...
        animate() {
            ...
            this.particles.forEach(p => {
                ...
                // Add border to confetti for sketchy look
                this.ctx.strokeStyle = '#1a1a1a';
                this.ctx.lineWidth = 2;

                if (p.shape === 'r') {
                    this.ctx.fillRect(-p.size/2, -p.size/2, p.size, p.size * 0.8);
                    this.ctx.strokeRect(-p.size/2, -p.size/2, p.size, p.size * 0.8);
                }
                else { 
                    this.ctx.beginPath(); 
